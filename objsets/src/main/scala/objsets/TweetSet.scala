...
class Empty extends TweetSet {
  ...
  val mostRetweeted: Tweet = new Tweet("", "", 0)
...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    def unionNonEmpty(nonem: NonEmpty): TweetSet = {
      if (elem.text < nonem.elem.text) nonem.left.union(this).incl(nonem.elem).union(nonem.right)
      else nonem.right.union(this).incl(nonem.elem).union(nonem.left)
    }
    
    that match {
      case set:NonEmpty => unionNonEmpty(set)
      case _ => this
    }
  }
  ...
  val mostRetweeted: Tweet = {
    ...
    if (elem.retweets < lMost.retweets) {
      if (lMost.retweets < rMost.retweets) rMost
      else lMost
    } else if (elem.retweets < rMost.retweets) rMost
    else elem
  }
